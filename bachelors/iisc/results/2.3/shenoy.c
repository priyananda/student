...
#define PROF_WEIGHT 0.8f
// #define PROF_WEIGHT 0.5f
// #define PROF_WEIGHT 0.3f
// #define PROF_WEIGHT 0.0f
...
void pp_calc_load(void){
	float x = (float)
		( kstat.cpu_user + kstat.cpu_system ) / (float) jiffies;
	float y = (float ) nr_running / (float) nr_tasks ;
	float load = x * 0.2f + y * 0.8f;
	pp_load = PROF_SCALE(load);
}
