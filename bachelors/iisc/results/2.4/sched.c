 ...
 */
/*
 *YEFP/Priyananda 2003-10-25 changed goodness()  and schedule() 
 * to implement lottery schduling 
 */
 ...
 */
		count = 0;
		r = pp_random( pp_base_tickets );
		while (p != &init_task) {
			real_tickets = p->pp_tickets * p->user->pp_tickets 
			real_tickets /= p->user->pp_tickets->given;
			count += real_tickets;
			if ( real_tickets > PP_COMPENSATION_MAX ){
				next = p;
				p->pp_tickets = p->pp_priority;
				break;
			}
			p->pp_tickets ++;
			if ( count > r ){
