 ...
 */
/*
 * Changed YEFP/Priyananda 2003-10-24 function do_fork() , to
...
{
	// Shenoy changed
	if( uid >= PP_SYSTEM_UID)
		up->pp_tickets = PP_SYSTEM_TICKETS;
	else
		up->pp_tickets = PP_USER_TICKETS;
	up->pp_tickets = current->pp_tickets;
...
int do_fork(unsigned long clone_flags, unsigned long usp, struct pt_regs *regs)
{
	...
	*p = *current;
	...
	p->prev_run = p;
	// Shenoy
	p->pp_tickets = p->priority;
	p->user->pp_tickets_given += p->pp_tickets;
	//
