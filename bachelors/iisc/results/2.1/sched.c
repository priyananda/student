 ...
 */
/*
 *  Priyananda Shenoy - YEFP - changes
 *  2003-08-15   schedule() changed to count idle ticks
 *  2003-09-21   goodness() modified to find cpu hog
 */
/*
 ...
 */
/*
 * This is what I want to screw up...PPS
 */
static inline int goodness(struct task_struct * p, struct task_struct * prev, int this_cpu)
{
	...
	if (weight) {
		...
		if (p->mm == prev->mm)
			weight += 1;
		// PPS - changed to use prof_priority instead of priority
		weight += p->prof_priority;
	}
 ...
 */
		...
		if (!c) {
			...
			for_each_task(p)
				p->counter = (p->counter >> 1) + p->prof_priority;
...
scheduling_in_interrupt:
	...
	*(int *)0 = 0;/* What the heck is this - PPS */
}
