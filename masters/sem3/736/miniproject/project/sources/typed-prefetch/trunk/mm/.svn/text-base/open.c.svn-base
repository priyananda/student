...
const char* get_ext(const char* tmp) {
  int i = 0;
  int last_dot = 0;
  int last_slash = 0;
  while(tmp[i] != '\0' && i<100) {
  ...
  if (i==100 || i - last_dot > 10) {
    return "err";
  }
  if (last_slash < last_dot) {
    return tmp + last_dot + 1;
  }
  return "---";
}
	...
	if (!IS_ERR(tmp)) {
		...
		if (fd >= 0) {
			...
			if (IS_ERR(f)) {
			  ...
			  strcpy(f->f_ext, get_ext(tmp));
				fsnotify_open(f->f_path.dentry);
				fd_install(fd, f);
				if (printdebugprftch == 1)
				  printk(KERN_DEBUG "PRFTCH_DBG:FM:%s:%lu:%s\n",
					 tmp, f->f_mapping->host->i_ino,f->f_ext);
			}
		}
		putname(tmp);
	}
